...
export default function ExportSection({
  ...
  const submissionPdf = (selectedPdfId ? pdfs.find(p => p.id === selectedPdfId && p.submissionId) : null) || checkedDone.find(p => p.submissionId) || null;
    ...
    <div className="v-section">
      ...
      {!isCollapsed && (
        <div className="export-inline-body">
          ...
          {/* ── Axiom Review Actions (only when a submission PDF is checked) ── */}
          {submissionId && (
            <div className="axiom-review-actions">
              <div className="axiom-review-label">Axiom Drawing Flow</div>
              <div className="axiom-review-btns">
                <button
                  className="axiom-btn axiom-btn-approve"
                  onClick={handleApprove}
                  disabled={approving || bouncing}
                  title="Mark submission as Approved — DT notified to produce final files"
                >
                  {approving ? 'Approving…' : '✓ Approve'}
                </button>
                <button
                  className="axiom-btn axiom-btn-bounce"
                  onClick={handleBounce}
                  disabled={bouncing || approving || checkedDone.length === 0}
                  title="Export annotated PDF and bounce submission back to DT with comments"
                >
                  {bouncing ? (progress || 'Bouncing…') : '↩ Bounce'}
                </button>
              </div>
              {actionResult && (
                <div className={`export-result ${actionResult.ok ? 'export-result-ok' : 'export-result-err'}`}
                  style={{ marginTop: 6 }}>
                  {actionResult.msg}
                </div>
              )}
